...
class EmotionScore(BaseModel):
    ...
    label: EmotionType = Field(
        ..., description="Emotion type", examples=["joy", "sadness", "anger"]
    )
    score: float = Field(
        ...,
        ge=0.0,
...
class SentimentResponse(BaseModel):
    ...
    scores: List[EmotionScore] = Field(
        ..., description="List of emotion scores sorted by confidence", min_length=1
    )
    status: StatusEnum = Field(
        default=StatusEnum.SUCCESS, description="Analysis status"
    )
    message: str = Field(
        default="Analysis completed successfully", description="Status message"
    )
...
class HealthResponse(BaseModel):
    ...
    status: HealthEnum = Field(..., description="Service status")
    version: str = Field(
        ..., pattern=r"^\d+\.\d+\.\d+$", description="API version", examples=["1.0.0"]
    )
    model_loaded: bool = Field(
        ..., description="Whether the ML model is loaded and ready"
    )
    model_name: Optional[str] = Field(
        default=None, description="Name of the loaded model"
    )
